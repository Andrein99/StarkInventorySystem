...
namespace StarkInventorySystem.Application.Orders.Commands.DeliverOrder
{
    public class DeliverOrderCommandHandler : IRequestHandler<DeliverOrderCommand, Result>
    {
        ...
        public async Task<Result> HandleAsync(DeliverOrderCommand request, CancellationToken cancellationToken = default)
        {
            try
            {
                // Cargar la orden
            ...
            {
                // Error inesperado
                return Result.Failure($"Un error ocurrió al entregar la orden: {ex.Message}");
            }
        }
    }
}
